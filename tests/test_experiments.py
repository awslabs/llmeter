...
class TestLoadTestResult:
    ...
    def test_load_test_result_init(self):
        ...
        load_test_result = LoadTestResult(
            results=results, test_name="test_experiment", output_path="/tmp/test"
        )
    ...
    def test_plot_results_html(self, mock_plot):
        ...
        load_test_result = LoadTestResult(
            results={}, test_name="test", output_path="/tmp/test"
        )
    ...
    def test_plot_results_show(self, mock_plot):
        ...
        load_test_result = LoadTestResult(
            results={}, test_name="test", output_path=None
        )
    ...
    def test_load_test_result_load_none_path(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="Load path cannot be None or empty"
        ):
...
class TestLatencyHeatmapExtended:
    ...
    def test_latency_heatmap_run_with_output_path(
        self, mock_runner_class, mock_create_prompt, tmp_path
    ):
        ...
        heatmap = LatencyHeatmap(
            endpoint=MagicMock(), source_file="test_file.txt", output_path=str(tmp_path)
        )
    ...
    @patch("llmeter.experiments.CreatePromptCollection")
    @patch("llmeter.experiments.Runner")
    def test_latency_heatmap_run_none_output_path(
        self, mock_runner_class, mock_create_prompt
    ):
        ...
        heatmap = LatencyHeatmap(
            endpoint=MagicMock(), source_file="test_file.txt", output_path=None
        )
...
class TestLoadTestExtended:
    ...
    @patch("llmeter.experiments.Runner")
    @pytest.mark.asyncio
    async def test_load_test_run_with_output_path_exception(
        self, mock_runner_class, mock_endpoint
    ):
