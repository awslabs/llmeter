...
def test_plot_heatmap_invalid_dimension(mock_px, sample_result: Result):
    ...
    with pytest.raises(ValueError, match="Dimension invalid_dimension not found in result"):
...
def test_scatter_histogram_2d(mock_px, sample_result: Result):
    ...
    fig = scatter_histogram_2d(sample_result, "num_tokens_input", "num_tokens_output", 10, 10)
...
def test_scatter_histogram_2d_invalid_dimension(mock_px, sample_result: Result):
    ...
    sample_result.get_dimension = MagicMock(side_effect=AttributeError("Invalid dimension"))
...
@patch("llmeter.plotting.plotting.go")
def test_average_input_tokens_clients_fig(mock_go, sample_load_test_result: LoadTestResult):
...
@patch("llmeter.plotting.plotting.go")
def test_average_output_tokens_clients_fig(mock_go, sample_load_test_result: LoadTestResult):
...
@patch("llmeter.plotting.plotting.go")
def test_latency_clients_invalid_dimension(mock_go, sample_load_test_result: LoadTestResult):
    ...
    for result in sample_load_test_result.results.values():
        result.get_dimension = MagicMock(side_effect=AttributeError("Invalid dimension"))
    
    with pytest.raises(ValueError, match="Invalid dimension"):
        latency_clients(sample_load_test_result, "time_to_first_token")  # Use valid type but mock error
...
def test_plot_load_test_results(mock_avg_out, mock_avg_in, mock_error, mock_rpm, mock_latency, sample_load_test_result: LoadTestResult):
...
def test_heatmap_initialization_with_responses(sample_responses):
    ...
    heatmap = Heatmap(responses=sample_responses, n_bins_input_tokens=5, n_bins_output_tokens=5)
...
def test_heatmap_initialization_with_result(sample_result):
    ...
    heatmap = Heatmap(result=sample_result, n_bins_input_tokens=5, n_bins_output_tokens=5)
