...
class MlflowCallback(Callback):
    ...
    def to_dict(self) -> dict:
        ...
        Returns:
            dict: Contains ``_callback_type``, ``step``, and ``nested``.
        """
        data = super().to_dict()
        # Exclude the mlflow version check side-effect attribute if present
        return data

    @classmethod
    def from_dict(cls, raw: dict, **kwargs) -> "MlflowCallback":
        """Reconstruct an MlflowCallback from a dict produced by ``to_dict()``.

        Args:
            raw: Dictionary with ``step`` and ``nested`` keys.

        Returns:
            MlflowCallback: The deserialized callback.
        """
        raw = dict(raw)
        raw.pop("_callback_type", None)
